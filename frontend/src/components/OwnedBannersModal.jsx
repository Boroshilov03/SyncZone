...
const OwnedBannersModal = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    // Effect to fetch owned banners when the component mounts or user changes
    const fetchOwnedBanners = async () => {
      ...
      if (error) {
        // If there's an error fetching user banners
      ...
      if (userBanners.length > 0) {
        ...
        if (bannersError) {
          // If there's an error fetching banners
      ...
      if (!currentActiveError && currentActive) {
        // If no error and currentActive exists
  ...
  const handleBannerSelect = async (bannerId) => {
    ...
    if (error) {
      // If there's an error updating or inserting
  ...
  const handleRemoveActiveBanner = async () => {
    // Function to handle removing the active banner
    if (!user) return;
    ...
    if (error) {
      // If there's an error deleting the active banner
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          ...
          {activeBannerId && (
            <TouchableOpacity
              onPress={handleRemoveActiveBanner}
...
const styles = StyleSheet.create({
  ...
  bannerContainer: {
    // Style for each banner container
